...
An object of genus two is on the left while its corresponding Reeb graph w.r.t. the height function is on the right. Note that each horizontal line parallel to the baseline is a level set w.r.t $f$.
