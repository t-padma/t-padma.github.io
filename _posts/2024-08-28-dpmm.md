...
Notice that models (1) and (2) assume that all components belong to the same family of a parametric distribution. If we assume that $i$-th component comes from density $f_i$ with parameter $\theta_i$, then we get a generalization of the model (1) which no longer has the expectation interpretation.
