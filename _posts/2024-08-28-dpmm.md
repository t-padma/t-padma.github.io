...
Note that model (3) involves the latent variable (or allocation variable) $Z_i$ since it is unobserved. Now, given a mixture model of the type (3), there are two common ways of inferring unknown parameters $(\theta_1, \cdots, \theta_G)^T, (\pi_1, \cdots, \pi_G)$ and $G$ (if it is unknown) - Frequentist approach vs Bayesian approach.
...
3. Bayesian vs frequentist mixtures (H random vs deterministic with unknown params)
...
Therefore, a mixture model can arbitrarily approximate complex densities if we can choose the right component distributions. 
