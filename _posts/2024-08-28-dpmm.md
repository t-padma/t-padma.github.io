...
\begin{equation} 
\mathbf{Y} \sim \sum_{i=1}^G \pi_i f(\mathbf{y}; \theta_i) \quad \text{ where } \sum_{j=1}^G \pi_j = 1 \text{ and } G < \infty (\#eq:mix)
\end{equation} 

Equivalently, mixture models can be interpreted as the expectation with respect to a mixing measure $H$. Define $H$ to be a measure with finite support that has weight $\pi_j$ at position $\theta_j$. Note that \@ref(eq:mix) and \@ref(eq:mix2) are equivalent representations of a mixture model.
\begin{equation} 
\mathbf{Y} \sim \int_{\Theta} f(\cdot ; \theta ) H(\mathrm{d}\theta)  (\#eq:mix2)
\end{equation} 
