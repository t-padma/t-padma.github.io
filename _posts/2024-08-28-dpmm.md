...
$$
\lim_{m\to\infty} \parallel f - h_m^{g} \parallel_{\mathcal{L_p}} = 0 \text{ where } \parallel f \parallel_{\mathcal{L_p}} \coloneqq \left( \int |\mathbf{1}_{\mathbb{X} f}|^p \mathrm{d} \lambda  \right )^{\frac{1}{p}} \text{ for Lebesgue measure } \lambda on $\mathbb{R}^n$ 
$$
