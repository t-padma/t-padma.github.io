...
$$
\mathbf{Y} \sim \sum_{i=1}^G \pi_i f(\mathbf{y}; \theta_i) \quad \text{ where } \sum_{j=1}^G \pi_j = 1 \text{ and } G < \infty    ---(1)
$$
...
$$
\mathbf{Y} \sim \int_{\Theta} f(\cdot ; \theta ) H(\mathrm{d}\theta)  ---(2)
$$
