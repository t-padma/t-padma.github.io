...
Notice that the rows (and columns) of $A_s$ correspond to the nodes compressed using sets $C_1, \cdots , C_m$. A directed edge from node $u_i$ to $u_j$ (in $G_s$) exists iff $A_s[i,j] > 0$. The directed edge $u_i \rihtarrow u_j$ is associated with the weight $A_s[i,j]$.
